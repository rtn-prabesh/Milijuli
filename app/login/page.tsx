  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        console.log(`[Login] User profile status: ${profile?.status}`);

        // 3. Routing logic based on status
        if (profile?.status === "pending") {
          router.push("/pending");
        } else if (profile?.status === "rejected") {
          setErrorMsg("Your account request was rejected. Please contact one of the admins.");
          await supabase.auth.signOut();
          setLoading(false);
        } else {
          // Approved! Redirect to core app
    ...
    <div className="min-h-screen flex flex-col justify-center items-center px-4 py-8 bg-slate-950 text-slate-100 relative overflow-hidden">
      ...
      <div className="w-full max-w-md glass rounded-2xl p-6 md:p-8 shadow-2xl border border-slate-200/5 bg-slate-900/40 relative z-10">
        <div className="text-center space-y-2 mb-8">
          ...
          <p className="text-xs text-slate-400">Log in to manage your Milijuli accounts</p>
        </div>
        ...
        <div className="mt-8 text-center text-xs text-slate-500">
          ...
          <Link href="/register" className="text-emerald-400 hover:underline font-semibold transition-all">
            Sign Up
          </Link>
        </div>
      </div>
